...
class MssqlEngine(EngineBase):
    ...
    def execute_workflow(self, workflow):
        if workflow.is_backup:
            ...
            backup_target_db = sys_config.get('mssql_backup_db', 'db1')
            if workflow.backup_tables:
                tables_need_backup = workflow.backup_tables.split(' ')
                for t in tables_need_backup:
                    backup_sql = f"""select * into {backup_target_db}.dbo.{t}_{now} from {t}"""
                    backup_result = self.execute(db_name=workflow.db_name, sql=backup_sql, close_conn=False)
